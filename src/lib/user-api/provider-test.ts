...
async function testApiyiProvider(apiKey: string): Promise<TestProviderResult> {
  const steps: TestStep[] = []
  const headers = { Authorization: `Bearer ${apiKey}` }

  try {
    const modelResponse = await fetch('https://api.apiyi.com/v1/models', {
      method: 'GET',
      headers,
      signal: AbortSignal.timeout(20_000),
    })
...
export async function testProviderConnection(payload: TestProviderPayload): Promise<TestProviderResult> {
  ...
  switch (apiType) {
    ...
    case 'apiyi':
      return testApiyiProvider(apiKey)
    default:
      return {
        success: false,
