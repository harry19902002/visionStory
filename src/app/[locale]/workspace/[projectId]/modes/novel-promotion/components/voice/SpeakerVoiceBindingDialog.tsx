...
import ArkVoicePickerDialog from '../assets/ArkVoicePickerDialog'
...
interface SpeakerVoiceBindingDialogProps {
    ...
    speaker: string
    projectId: string
    initialSpeakerVoice?: { provider?: string, voiceId?: string, speed?: number, pitch?: number, vol?: number, instruction?: string, [key: string]: unknown }
    onClose: () => void
...
export default function SpeakerVoiceBindingDialog({
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            } else if (initialSpeakerVoice?.provider === 'ark-speech' || initialSpeakerVoice?.provider === 'ark') {
                setArkVoiceId(initialSpeakerVoice.voiceId || 'zh_female_shuangkuaisisi_moon_bigtts')
                setSpeed(initialSpeakerVoice.speed ?? 1.0)
                setPitch(initialSpeakerVoice.pitch ?? 0)
                setVol(initialSpeakerVoice.vol ?? 1.0)
                setArkInstruction(initialSpeakerVoice.instruction ?? '')
            } else {
                setMinimaxVoiceId('male-qn-qingse')
                setArkVoiceId('zh_female_shuangkuaisisi_moon_bigtts')
                setArkInstruction('')
                setSpeed(1.0)
                setPitch(0)
                setVol(1.0)
            ...
            <>
                ...
                <div className="fixed z-[10000] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 glass-surface-modal w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--glass-stroke-base)] bg-[var(--glass-bg-surface-strong)]">
                        <div className="flex items-center gap-2 min-w-0">
                            <AppIcon name="mic" className="w-5 h-5 text-[var(--glass-tone-info-fg)] shrink-0" />
                            <h2 className="font-semibold text-[var(--glass-text-primary)] truncate">火山 Ark 配音设置 - {speaker}</h2>
                        </div>
                        <button onClick={handleClose} className="glass-btn-base glass-btn-soft p-1 text-[var(--glass-text-tertiary)] shrink-0">
                            <AppIcon name="close" className="w-5 h-5" />
                        </button>
                    ...
                    <div className="p-5 space-y-5">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-[var(--glass-text-secondary)] mb-2">默认情绪 (可选)</label>
                            <input type="text" placeholder="例如：用悲伤的语气" value={arkInstruction} onChange={e => setArkInstruction(e.target.value)} className="w-full bg-[var(--glass-bg-surface)] border border-[var(--glass-stroke-base)] rounded-lg px-3 py-2 text-sm text-[var(--glass-text-primary)] focus:outline-none focus:border-[var(--glass-stroke-focus)]" />
                        </div>
                    </div>
                    <div className="flex gap-2 p-4 border-t border-[var(--glass-stroke-base)] bg-[var(--glass-bg-surface-strong)]">
                        <button onClick={handleClose} className="glass-btn-base glass-btn-secondary flex-1 py-2 rounded-lg text-sm">{t('cancel', { defaultValue: '取消' })}</button>
